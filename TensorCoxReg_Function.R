...
CoxTensor_Est <- function(DATA, n_R, opt, max_ite, tol){
  ...
  for (ite_index in 1:max_ite) {
    ...
    B_1_new <- matrix(Sur_Coef(t(X %w% B_2_new), y, status, 
                               offset_vec), n_P, n_R)
    G <- matrix(B_1_new[n_R, n_R], n_R, n_R) # C in  B_1_new
    B_new <- B_1_new %*% t(B_2_new) # The new B obtained by the outer product of new B1 and new B2
  ...
  c_index <- cIndex(time = y, event = status, 
                    risk_score = exp(W %*% beta + X %hp% B))
  return(list(b_Est = beta, B_Est = B, B1_Est = B_1, B2_Est = B_2, 
              c_index = c_index, ite = ite_index))
}
...
Calculate_IC <- function(DATA, n_R, B, beta, df){
  ...
  return(list(AIC = AIC, BIC = BIC, BIC_NoE = BIC_NoE))
}
